...
class VitaeLangXWindow:
    ...
    def initialize_app_backend(self):
        ...
        if self.backend_manager:
            ...
            print(
                "CRITICAL ERROR: BackendManager could not be instantiated in initialize_app_backend.")
    ...
    def navigate_to_page(self, page_name, **kwargs):
        if self.backend_manager is None:
            print(
                "ERROR in navigate_to_page: backend_manager is None. Initialization might have failed.")
            for widget in self.content_frame.winfo_children():
                widget.destroy()
            error_label = ctk.CTkLabel(
                self.content_frame, text="Critical Error: Backend not available.", font=ctk.CTkFont(size=20))
        ...
        common_args = {"parent": self.content_frame, "navigate_callback":
                       self.navigate_to_page, "backend_manager": self.backend_manager}
        ...
        if page_name == "home":
            self.current_page = HomePage(
                self.content_frame, self.navigate_to_page)
        elif page_name == "about":
            self.current_page = AboutPage(
                self.content_frame, self.navigate_to_page)
        elif page_name == "creator":
            self.current_page = CreatorPage(
                self.content_frame, self.navigate_to_page)
        elif page_name == "opening":
            self.current_page = OpeningPage(
                self.content_frame, self.navigate_to_page)
            ...
            print(
                f"Warning: Unknown page name '{page_name}' in navigate_to_page.")
            self.current_page = HomePage(
                self.content_frame, self.navigate_to_page)
